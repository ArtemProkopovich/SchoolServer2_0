...
public class ScheduleServiceTest {
    ...
    public void getPupilDayLessons() throws Exception {
        List<SchedulePupilLesson> lessonList = service.GetPupilDayLessons(1, new GregorianCalendar(2015, 00, 01).getTime());
        Assert.assertNotNull(lessonList);
        Assert.assertTrue(lessonList.size() > 0 && lessonList.get(0).getSubject() != null && lessonList.get(0).getLesson()
                != null && lessonList.get(0).getTeacher() != null);
    }

    @Test
    public void getTeacherDayLessons() throws Exception {
        List<ScheduleTeacherLesson> lessonList = service.GetTeacherDayLessons(1, new GregorianCalendar(2015, 00, 01).getTime());
        Assert.assertNotNull(lessonList);
    ...
    public void getClassDayLessons() throws Exception {
        List<SchedulePupilLesson> lessonList = service.GetClassDayLessons(1, new GregorianCalendar(2015, 00, 01).getTime());
        Assert.assertNotNull(lessonList);
        Assert.assertTrue(lessonList.size() > 0);
    }
